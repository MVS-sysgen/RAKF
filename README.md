...
### Managing Profiles with ADDSD, RDEFINE, PERMIT and RDELETE
...
- **PERMIT** – Grants access to a resource of a certain class.
  ...
  **PERMIT** sorts the records in alphabetical order and remove the comments records.
...
```
ADDSD profile UACC(NONE|READ|UPDATE|ALTER)
Required: 'profile'. Default: UACC(NONE)
An alias of ADDSD is AD.

RDEFINE profile CLASS(class) UACC(NONE|READ|UPDATE|ALTER)
...
Required: 'profile', CLASS, ID.
If ID(ALL) is specified, all records of 'profile' are deleted
including the Universal Access record.
```
